...
linear_regression = function(values1, values2) {
    ...
    lin_reg= lm (y~x, data.frame(x=values1 ,y=values2))
    results=summary(lin_reg)
    slope=results$coefficients[2,1]
    intercept=results$coefficients[1,1]
    p.value=results$coefficients[2,4]
...
calculate_trait_at_internal_node = function(trait_child1, trait_child2, branch_length_to_child1, branch_length_to_child2) {
    ...
    #   branch_length_to_child1: branch length leading to first child (numeric)
    #   branch_length_to_child2: branch length leading to second child (numeric)

    num1=trait_child1*(branch_length_to_child2/(branch_length_to_child1+branch_length_to_child2))
    num2=trait_child2*(branch_length_to_child1/(branch_length_to_child1+branch_length_to_child2))
    trait_at_internal_node=num1+num2
...
calculate_corrected_branch_length = function(node, child1, child2, corrected_branch_lengths) {
    ...
    t=corrected_branch_lengths[node]
    corrected_length=((t1*t2)/(t1+t2))+t
...
get_contrasts_in_subtree = function(node, tree, description) {
    ...
    # for the child nodes, saving the updated description.
      ...
      results1=calculate_trait_and_contrast(child1, child2, updated_description$traits[,1], updated_description$corrected_branch_lengths)
      updated_description$traits[node,1]=results1$trait_value
      updated_description$normalized_contrasts[node,1]=results1$contrast
      results2=calculate_trait_and_contrast(child1, child2, updated_description$traits[,2], updated_description$corrected_branch_lengths)
      updated_description$traits[node,2]=results2$trait_value
      updated_description$normalized_contrasts[node,2]=results2$contrast
...
get_trait_correlation = function(newick_tree, traits_at_tip) {	
    ...
    n=length(traits_at_tip$trait1)
    raw_data_regression=linear_regression(updated_description$traits[1:n,1], updated_description$traits[1:n,2])

    # Perform linear regression on the normalized contrasts and get the slope, intercept, P-value, and R2-value
    m=length(updated_description$corrected_branch_lengths)
    contrasts_regression=linear_regression(updated_description$normalized_contrasts[(n+1):m,1], updated_description$normalized_contrasts[(n+1):m,2])
